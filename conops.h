...
typedef int (*transition_handler_t)(struct conops_fsm *ctx,
                                    const struct conops_event *ev,
                                    const uint16_t transition_to);
typedef int32_t (*event_mapper_t)(const struct conops_fsm *ctx,
                                  const struct conops_event *ev);
typedef void (*state_change_callback_t)(const struct conops_fsm *ctx,
                                        uint16_t state);
typedef void (*event_callback_t)(const struct conops_fsm *ctx,
                                 const struct conops_event *ev);
...
struct conops_fsm {
  ...
  void *user_data;
  const state_change_callback_t *state_enter;
  const state_change_callback_t *state_exit;
  event_mapper_t ev_mapper_fn;
};
